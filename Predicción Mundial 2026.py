...
st.markdown("""
    ...
    /* --- TABLA DE POSICIONES EMBELLECIDA (NEGRO TOTAL) --- */
    [data-testid="stTable"] { 
    ...
    /* --- INPUTS SIN CRUCES NI CAJAS BLANCAS --- */
    [data-testid="stInputClearButton"], button[aria-label="Clear input"] { display: none !important; }
    ...
    <div style="text-align: center; padding: 20px 0;">
        <span class="titulo-personalizado">PRODE MUNDIAL 2026</span>
    </div>
...
# --- 3. INTERFAZ ---
tab_p, tab_r, tab_c = st.tabs(["🔮 PREDICCIÓN", "📈 REAL", "🎯 PUNTOS"])

with tab_p:
    puede_p = ahora < fecha_limite
    st.text_input("👤 **NOMBRE DEL PARTICIPANTE:**", key="user_name", placeholder="Ej: Juan de Comodoro")
        ...
        with c1:
            for i in range(len(equipos)):
                for j in range(i + 1, len(equipos)):
                    ...
                    cols[2].markdown("<p style='text-align:center; color:#FF8C00; font-weight:bold;'>-</p>", unsafe_allow_html=True)
    ...
    st.button("🚀 GUARDAR PRONÓSTICO", use_container_width=True, disabled=not puede_p)
